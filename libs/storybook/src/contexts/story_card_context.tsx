import type { ReactNode } from 'react'
import { createContext } from 'react'
import type { RequiredPick } from 'type-plus'
import type { StoryCardProps } from '../components/story_card.js'

/**
 * Payload for adding a card to the story card context.
 * Matches the shape of card props with `status` required.
 */
export type StoryCardContextCard = RequiredPick<Omit<StoryCardProps, 'children'> & { content?: ReactNode }, 'status'>

export interface StoryCardContextValue {
	addCard: (card: StoryCardContextCard) => string
	removeCard: (id: string) => void
}

export const StoryCardContext = createContext<StoryCardContextValue | null>(null)
